...
public class ManualController {
	...
	public CreatePaymentResultDTO creatPaymentManualPadding(@RequestBody List<PayManualSetterDTO> manualDTO)
		...
		for (PayManualSetterDTO dto : manualDTO) {
			...
			if(listPayInvoiceManualEntity.size() > 0) {
				...
				if(!"".equals(pay.getServiceType()) && !"MANUAL".equals(pay.getServiceType()) && pay.getServiceType() != null) {
					mapGLList =	mapGLServiceTypeRepo.findByServiceCode(pay.getServiceType());
				}
			}
			if ("Billing".equals(dto.getSource()) || "RECEIVEGATEWAY".equals(dto.getSource())) {
				...
				if (profile == null) {
					...
					try {
						...
						inv.setIssueDt(
								response.getInvoiceHeaderList().get(0).getIssueDate().toGregorianCalendar().getTime());
						inv.setDueDt(
								response.getInvoiceHeaderList().get(0).getDueDate().toGregorianCalendar().getTime());
						...
						if ("RECEIVEGATEWAY".equals(dto.getSource())) {
							...
							for(TrsMethodManualEntity trsMethodManualEntity : listTrsMethodManualEntity){
								if("CR".equals(trsMethodManualEntity.getCode())) {
									List<MethodCreditCardManual> mcm =	creditCardManualRepository.findByMethodManualId(trsMethodManualEntity.getMethodManualId());
									...
									List<MethodChequeManual> mcm = chequeManualRepository.findByMethodId(trsMethodManualEntity.getMethodManualId());
	...
	public void findBussinessPlaceJSON(ModelMap modelMap) {
		List<Map<String, Object>> a = episJdbcTemplate.queryForList("SELECT DISTINCT PROPERTY_1 as property1  FROM MASTER_DATA m WHERE Group_key = 'BUSINESS_AREA' order by PROPERTY_1 asc");
