...
<title>EPIS Back Office : Offline Pos Management UI.</title>
...
<script>
	...
	function rowsFormatter(id) {return '<a id="posEdit" style="cursor:pointer" onClick="findPosOfflineDetailById(\''+ id +'\')" class="btn-sm btn-info"><i class="glyphicon glyphicon-edit"></i> แก้ไข</a>'; }
	function findPosOfflineDetailById(id) {
		$.get("../findPosOfflineDetailById.json", { "id": id }, function(res) {
			$('#searchPosPanel').addClass('hide'); $('#displayPosPanel').addClass('hide');  $('#createPosPanel').removeClass('hide');  $('#resetCreate').addClass('disabled');
			$("#id").val(res.id); $("#no").val(res.no); $("#name").val(res.name); $("#mac").val(res.mac); $("#contact").val(res.contact); $("#telephone").val(res.telephone); $("#isPositive").val(res.isPositive); $("#description").val(res.description); 
		});
	}	
</script>	
</head>
					...
					<table id="posTable" data-toggle="table" data-toolbar="#PosToolbar" data-search="true" data-show-pagination-switch="true" data-show-refresh="true" data-show-toggle="true" data-show-columns="true" data-show-export="true" data-pagination="true" data-height="400" data-cache="false">
                       <thead>
                           <tr>
								...
								<th data-field="contact" data-align="center">ชื่อ - สกุล ผู้ติดต่อ</th>
								<th data-field="telephone" data-align="center">เบอร์โทรติดต่อ</th>
						...
						<div class="tab-content">
							<div role="tabpanel" class="tab-pane active" id="tab_1">
								<div class="form-horizontal">
									...
									<div class="form-group">
										<label class="control-label col-sm-2">ชื่อ - สกุล ผู้ติดต่อ :</label>
										<div class="col-sm-2"><input class="form-control" id="contact" name="contact"></div>
										<label class="control-label col-sm-2">เบอร์โทรติดต่อ :</label>
										<div class="col-sm-2"><input class="form-control" id="telephone" name="telephone"></div>
										<div class="col-sm-3"></div>
									</div>
									<div class="form-group">
										<label class="control-label col-sm-2">ชื่อหน่วยงานรับชําระ :</label>
...
$(document).on("click", "a#searchPos", function() {
	...
	if($('#noForSh').val() == '' && $('#nameForSh').val() == '' && $('#shopForSh').val() == '') {
		...
		$("#posTable").bootstrapTable("refresh", { url: "../findPosOfflineByBranchAttribute.json?no="+ $("#noForSh").val() +"&name="+ $("#nameForSh").val() +"&shop="+ $("#shopForSh").val() });
	...
	if($('#no').val() == '' && $('#name').val() == '' && $('#mac').val() == '' && $('#shop').val() == null) {
		...
		$.ajax({
			type: "POST",
			url: "../savePosOfflineByBranchAttribute.json",
