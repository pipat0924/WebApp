...
//		queryBuilder.append(" ) aaa GROUP BY aaa.UPDATEUSER ORDER BY aaa.UPDATEUSER asc ");
	...
	public ReportPaymentDTO getUnCloseEndOfDayReportJSON(HttpServletRequest request,
		...
		System.out.println("printPdf = "+ printPdf);
		System.out.println("startDate = "+ searchDate);
		System.out.println("toDate = "+ searchDateTo);
...
//				givenDateTo = AppUtil.formatter_EN.parse(convertDateString(searchDateTo));
//			} catch(Exception e) {
//				givenDateFrom = new Date();
//				givenDateTo = new Date();
//			}
//			
...
//		}
		...
		final ReportPaymentDTO reportPaymentDTO = new ReportPaymentDTO();
		StringBuilder queryBuilder = new StringBuilder();
		queryBuilder.append(" SELECT DISTINCT TO_CHAR(cr.receiptdttm, 'dd/MM/yyyy') AS receiptdttm, mp.posname , mf.username, (mf.officergivenname ||' '|| mf.officerfamilyname) AS payfullname, ms.shopno, ms.shopname ");
		queryBuilder.append(" FROM correceipt cr ");
		queryBuilder.append(" INNER JOIN corpayment cp ON cp.paymentid = cr.paymentid ");
		...
		episJdbcTemplate.query(queryBuilder.toString(), new RowCallbackHandler(){
			...
			public void processRow(ResultSet row) throws SQLException {
				ReportPayment reportPayment = new ReportPayment();
				reportPayment.setReceiptDate(row.getString(1));
//				Long posid = row.getLong(2);
//				System.out.println("====================== = "+posid);
//				if(posid > 0) {
//				String posname =	reportService.findPosNameGetByPosID(posid);
//					if(!posname.equals("") && posname != null) {
//						reportPayment.setPosName(posname);
//					}
//				}
				reportPayment.setPosName(row.getString(2));
				reportPayment.setPaymentUser(row.getString(3));
				reportPayment.setPayFullname(row.getString(4));
				reportPayment.setShopNo(row.getString(5));
				reportPayment.setShopName(row.getString(6));
				reportPaymentDTO.addData(reportPayment);
			}
		});
		if(AppUtil.isStringHasValue(printPdf)) {
