...
//		bounceChequeList = payBounceChequeRepository.searchPaySAP(arCode, docHead);
		...
		SapDebtDao sapDao = new SapDebtDao();
		...
		listSap = getDataAccountSapDebt(arCode, docHead);
		
		if(listSap.size() >0) {
			listSapCor = getDataAccountCorreCeipt(listSap.get(0).getAccountNo());
			for (SapDebtDTO sapbean :listSap) {
				payBounceChequeDTO = new PayBounceChequeDTO();
				if (sapbean.getLocalBalanceDue()!=null) {
					payBounceChequeDTO.setRefDate(sapbean.getRefSapPeriod());
					payBounceChequeDTO.setDocHead(sapbean.getDocHeaderTextInv());
					payBounceChequeDTO.setServiceKey3(sapbean.getRef3ServiceName());
					payBounceChequeDTO.setCurrency(sapbean.getCurrencyCodeAr());
					payBounceChequeDTO.setAmountARin(new Double(sapbean.getLocalBalanceDue()));
					payBounceChequeDTO.setRateChange(new Double(sapbean.getExchRateAr()));
					payBounceChequeDTO.setAmountARout(new Double(sapbean.getTransBalanceDue()));
					payBounceChequeDTO.setCurrencyChangeDate(sapbean.getExchRateDateAr()); // formatDatePass
					payBounceChequeDTO.setDocNo(sapbean.getSapdocNo());
					payBounceChequeDTO.setAccountYear(Integer.valueOf(sapbean.getFiscalYear()));
					payBounceChequeDTO.setDocDate(sapbean.getDocDateAr()); // formatDatePass
					payBounceChequeDTO.setPassDate(sapbean.getPostingDateAr());
					payBounceChequeDTO.setVatCode(sapbean.getTaxCode());
					payBounceChequeDTO.setMessage(sapbean.getAwText());
					payBounceChequeDTO.setArAccountCode(sapbean.getAccountNo());
					payBounceChequeDTO.setArName(sapbean.getCustomerName());
					payBounceChequeDTO.setGlAccount(sapbean.getGlAccount());
					payBounceChequeDTO.setTaxId(sapbean.getTaxCode());
					payBounceChequeDTO.setArGroup(sapbean.getCustomerGroupCode());
					payBounceChequeDTO.setRegionKey1(sapbean.getRef1SapRegionCode());
					payBounceChequeDTO.setBranchAR(sapbean.getCustomerBranchCode());
					payBounceChequeDTO.setAddress(sapbean.getCustomerHomeStreet());
					payBounceChequeDTO.setRecordStatus(sapbean.getRecordStatus());
					payBounceChequeDTO.setAmountPay(new BigDecimal(sapbean.getTransBalanceDue()));
					payBounceChequeDTO.setAmountTotalPay(new BigDecimal(sapbean.getTransBalanceDue()));
					payBounceChequeDTO.setCurrencyCode(sapbean.getCustomerCountryKey());
...
//					historyARDTO.setReceiptDate(corBean.getReceiptdttm());
	...
	@Transactional
	public BounceChequeDTO searchPaySapHistory(String arCode, String docHead) {
		BigDecimal vatRate = new BigDecimal(masterDataRepository.findByKey(AppConstants.VAT_RATE).get(0).getValue());
		bounceChequeDTO = new BounceChequeDTO();
		detailARCustomerDTO = new DetailARCustomerDTO();
		payBounceChequeDTO = new PayBounceChequeDTO();
		...
		listSap = getDataAccountSapDebtDetail(arCode);
...
//					historyARDTO.setReceiptDate(corBean.getReceiptdttm());
	...
	public List<SapDebtDTO> getDataAccountSapDebt(String arCode, String docHead ) {
		...
		try {
			...
			sql.append("select * from SAP_DEBT where 1=1 ");
			if(arCode != null && !"".equals(arCode)) {
				sql.append("and ACCOUNT_NO =? ");
				list.add(arCode.trim());
			}if(docHead != null &&!"".equals(docHead) ) {
				sql.append("and DOC_HEADER_TEXT_INV =? ");
				list.add(docHead.trim());
			}
			sql.append("order by SAPDOC_NO ");
	...
	public List<SapDebtDTO> getDataAccountSapDebtDetail(String arCode) {
		List<SapDebtDTO> listSap = new ArrayList<SapDebtDTO>();
		try {
			StringBuffer sql = new StringBuffer();
			sql.append("select s.* from SAP_DEBT s where s.ACCOUNT_NO =?");
			listSap = (List<SapDebtDTO>) episJdbcTemplate.query(sql.toString(), new Object[]{arCode}, new BeanPropertyRowMapper<SapDebtDTO>(SapDebtDTO.class));
		}catch (Exception e) {
			e.printStackTrace();
		}
		return listSap;
	...
	public List<SapCorreceiptDTO> getDataAccountCorreCeipt(String accuntNo) {
		...
		try {
			StringBuffer sql = new StringBuffer();
			sql.append("SELECT CO.* ,TM.* ,TM.VAT AS VATTM ,DE.REF3_SERVICE_NAME  ,DE.CURRENCY_CODE_AR ");
			sql.append("FROM CORRECEIPT CO ");
			sql.append("INNER JOIN TMPINVOICE TM ON CO.PAYMENTID = TM.PAYMENTID ");
			sql.append("INNER JOIN SAP_DEBT DE  ON TM.INVOICENO = DE.DOC_HEADER_TEXT_INV ");
			sql.append("WHERE CO.ACCOUNTNO =? ");
	...
	public BounceChequeDTO sapPaymentSummr(BounceChequeDTO bean) throws Exception {
		...
		List<SapDebtDTO> sapListt = new ArrayList<SapDebtDTO>();
		...
		if (chk) {
			...
			settlePaymentDTO.setAmount((BigDecimal) decimalFormat.parse(bean.getPayBounceChequeDTO().getPreItemsDiscount().toString()));
			...
			settlePaymentDTO.setPayAmount((BigDecimal) decimalFormat.parse(bean.getPayBounceChequeDTO().getPreItemsDiscount().toString()));
			// settlePaymentDTO.setPaymentCase(bean.getPayBounceChequeDTO().getPaymentCase());
			settlePaymentDTO.setReceiveAmount((BigDecimal) decimalFormat.parse(bean.getPayBounceChequeDTO().getInputReceived().toString()));
			settlePaymentDTO.setRemainAmount(BigDecimal.ZERO);
			// settlePaymentDTO.setRemark(remark);
			settlePaymentDTO.setTotalPaid((BigDecimal) decimalFormat.parse(bean.getPayBounceChequeDTO().getTotalCharge().toString()));
			settlePaymentDTO.setVatAmount((BigDecimal) decimalFormat.parse(bean.getPayBounceChequeDTO().getVat().toString()));
			...
			customer.setCustType(bean.getDetailARCustomerDTO().getArGroup().equals("001") ? "INDIVIDUAL" : "ORGANIZATION"); // "INDIVIDUAL"// mock
			...
			customer.setExchangeRate(new BigDecimal(bean.getPayBounceChequeDTO().getRateChange()));
			for (int i = 0; i < bean.getPayBounceChequeDTOList().size(); i++) {
				...
				invoice.setBalanceDue((BigDecimal) decimalFormat.parse(bean.getPayBounceChequeDTOList().get(i).getAmountTotalPay().subtract(bean.getPayBounceChequeDTOList().get(i).getAmountPay()).toString()));
				invoice.setReceived((BigDecimal) decimalFormat.parse(bean.getPayBounceChequeDTOList().get(i).getAmountPay().toString()));
				invoice.setChange(BigDecimal.ZERO);
				DateUtil.convertToDate(bean.getPayBounceChequeDTOList().get(i).getPassDate());
//				invoice.setIssueDt(fmString2Date(bean.getPayBounceChequeDTOList().get(i).getPassDate())); //ยังไม่แก้
				invoice.setIssueDt(DateUtil.convertToDate(bean.getPayBounceChequeDTOList().get(i).getPassDate()));
				...
				SapDebtDTO sapBean = new SapDebtDTO();
				sapBean.setLocalTotalPaid(bean.getPayBounceChequeDTOList().get(i).getTotalChargeBath());
				sapBean.setLocalVatPaid(bean.getPayBounceChequeDTOList().get(i).getVatBath());
				sapBean.setTransTotalPaid(bean.getPayBounceChequeDTOList().get(i).getAmountPay().doubleValue());
				sapBean.setTransVatPaid(bean.getPayBounceChequeDTOList().get(i).getVat());
				sapBean.setDocHeaderTextInv(bean.getPayBounceChequeDTOList().get(i).getDocHead());
				sapBean.setAccountNo(bean.getPayBounceChequeDTOList().get(i).getArAccountCode());
				sapBean.setLocalBalanceDue(bean.getPayBounceChequeDTOList().get(i).getLocalBalDueSap());
				sapBean.setTransBalanceDue(bean.getPayBounceChequeDTOList().get(i).getTransBalDueSap());
				sapListt.add(sapBean);
			}
