...
<html>
    <head>
        <title>รายงานการยกเลิกการรับชำระเงิน (ตัวแทนรับชำระ)</title>
    ...
    <body>
        ...
        <section class="container-fluid menu">
            ...
            <div class="row">
                <div class="col-md-12">
                    <ol class="breadcrumb">
                    	<li><i>รายงาน (ตัวแทนรับชำระ)</i></li>
                        <li class="active"><i>รายงานการยกเลิกการรับชำระเงิน (AGENT)</i></li>
                    </ol>
                    <div class="panel panel-default">
                    	<div class="panel-heading">ข้อมูลการยกเลิกการรับชำระเงิน  (AGENT)</div>
                    	<div class="panel-body">
                    		<div class="row">
                    			<div class="col-md-12" style="margin-left:-2em">
                    			<form id=documentReportForm method="post" class="form-horizontal" role="form">
									<div class="form-group">
										<label class="control-label col-sm-2">วันที่ยกเลิกการรับชำระ:</label>
										...
										<label class="control-label col-sm-2" style="width: 9.6%;">ถึงวันที่:</label>
                                         <div class="col-sm-2" style="width:13%">
										...
										<label class="control-label col-sm-2" style="width: 13%;">เลขที่ใบเสร็จรับเงิน:</label>
                                      ...
                                      <div class="form-group">
                                        ...
                                        <label class="control-label col-sm-2" style="width:11%">ประเภทตัวแทน:</label>
										<div class="col-sm-2">
											<select class="form-control" id="sourceType" name="sourceType"></select>
										</div>	
										<label class="control-label col-sm-1" style="width:8%">เจ้าหน้าที่:</label>
			            ...
			            <table id="CancelPaymentReport"
				          ...
				          <thead>
					          <tr>
				                   ...
				                   <th data-align="center" data-field="agentName" data-sortable="true">ชื่อตัวแทน</th>
				                   <th data-align="center" data-field="receiptNo" data-sortable="true">เลขที่ใบเสร็จรับเงิน/ใบกำกับภาษี</th>
				                   <th data-align="center" data-field="taxId" data-sortable="true">เลขที่บัตรประจำตัวผู้เสียภาษีของตัวแทน</th>
				                   <th data-align="left" data-field="ref1" data-sortable="true">Ref no.1</th>
								   <th data-align="left" data-field="ref2" data-sortable="true">Ref no.2</th>
				                   <th data-align="left" data-field="paymentCash" data-sortable="true">วิธีการชำระเงิน</th>
				                   <th data-align="left" data-field="paymentUser" data-sortable="true">เจ้าหน้าที่รับชำระ</th>
				                   <th data-align="left" data-field="cancelByUser" data-sortable="true">เจ้าหน้าที่ยกเลิก</th>
				                   <th data-align="center" data-field="receiptDate" data-sortable="true">วันที่รับชำระ</th>
				                   <th data-align="center" data-field="cancelDate" data-sortable="true">วันที่ยกเลิกการรับชำระ</th>
				                   <th data-align="right" data-field="charge" data-formatter="currencyFormatter" data-sortable="true">มูลค่าบริการ</th>
				                   <th data-align="right" data-field="vat" data-formatter="currencyFormatter" data-sortable="true">ภาษีมูลค่าเพิ่ม</th>
				                   <th data-align="right" data-field="totalCharge" data-formatter="currencyFormatter" data-sortable="true">จำนวนเงินรวม</th>
					         	   <th data-align="right" data-field="reason"  data-sortable="true">เหตุผลที่ยกเลิก</th>
            ...
            <form action="../printCancelPaymentReport.pdf" method="post" target="_printForm">
            	...
            	<input type="hidden" id="serviceType" name="serviceType" value="AGENT"/>
            </form>
            <form action="../printCancelPaymentReport.xls" method="post" target="_printForm">
            	<input type="hidden" id="printPdf" name="printPdf" value="1"/>
            	<input type="hidden" id="startDate" name="startDate"/>
            	<input type="hidden" id="endDate" name="endDate"/>
            	<input type="hidden" id="invNo" name="invNo"/>
            ...
            <script type="text/javascript">
            	$(document).on("click", "a#inquiryReport", function() {
            		$("#CancelPaymentReport").bootstrapTable("refresh", 
            				{ url: "../getCancelPaymentReport.json?printPdf=0"
            						...
            						, cache: false });
            			setTimeout(function() { 
            				$("#CancelPaymentReport").bootstrapTable("resetView")
            		}, 200);
...
//             		document.forms[1].period.value = $("#period option:selected").val();
            		document.forms[1].invNo.value = $("#invNo").val().trim();
            		document.forms[1].shop.value = $("#shop option:selected").val();
            		document.forms[1].sourceType .value = $("#sourceType option:selected").val();
            		...
            		document.forms[2].invNo.value = $("#invNo").val().trim();
            		document.forms[2].shop.value = $("#shop option:selected").val();
            		document.forms[2].sourceType.value = $("#sourceType option:selected").val();
            	...
            	var view = (function($){
            		...
            		console.log("roleName : "+roleName);
            			...
            			$.each(array,function(i,o){
            				...
            				if(o.companyName != null) {
            					if(o.code != 'MNP'){
	            					obj.elem.append('<option data-index="'+ i +'" data-key="'+ o.code +'" value="'+ o.code +'">'+ o.name +'</option>')
            					}
            				} else {
            					...
            					if (o.no != null) {
            						...
            						if(roleName == "Suppervisor" || roleName == "AGENT"){
            							$("#shop").val(tempShop)
            		...
            		var strComboValue = [ 
            			...
            			new DropDown("#sourceType").init("../findAllAgentType.json");
            			if(roleName == "Suppervisor" || roleName == "AGENT"){
            				new DropDown("#shop").disable();
            				}
            		return this;
