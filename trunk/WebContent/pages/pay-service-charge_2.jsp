...
<body>
	...
	<section class="container-fluid menu">
		...
		<ul class="list-inline pull-right list-menu-set">
			<li><a href="pay-service-charge.jsp?addInvoice" class="btn btn-link"><span class="glyphicon glyphicon-user"></span> เพิ่มข้อมูลลูกค้า</a></li>
...
$(function(){
	...
	for (var i = 0, m = billingList.length; i < m; i++) {
		for (var j = 0, n = billingList[i].invoiceList.length; j < n; j++) {
			...
			deduct += calculateVatWT(invoice.balanceDue, invoice.received, invoice.vatRate, invoice.deduction, invoice.totalCharge, wtAvg);
			...
			//discount += invoice.afterSaleDiscount;//comment by kao
...
// 				+ '<td class="text-right">'+ view.utils.numberFormat(v.afterSaleDiscount) +'</td>'
				...
				+ '<td class="text-right">'+ view.utils.numberFormat(calculateVatWT(v.balanceDue, v.received, v.vatRate, v.deduction, v.totalCharge, v.wtAvg) ) +'</td>'
