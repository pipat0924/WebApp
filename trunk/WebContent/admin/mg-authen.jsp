...
// 	function principalFormatter(val) { return val.name; }
	...
	function findDetailById(id) {
		$.get("../findEnumById.json", { "id": id }, function(res) {
			...
			$("#id").val(res.data[0].id); $("#code").val(res.data[0].code); $("#isPositive").val(res.data[0].isPositive); 
		});
	}	
		...
		<div class="row">
			<div class="col-md-12 tab-modefile">
							...
							<div role="tabpanel" class="tab-pane active" id="tab_1">
								<div class="form-horizontal">
									<div class="form-group">
										<label class="control-label col-sm-3">รหัสผู้ใช้งาน :</label>
										<div class="col-sm-3"><input class="form-control" id="codeCrt" name="codeCrt"></div>
										<div class="col-sm-6">
				...
				<div id="displayPanel" class="hide tab-content">
					...
					<table id="searchResultTable" data-toggle="table" data-toolbar="#authenToolbar" data-search="true" data-show-pagination-switch="true" data-show-refresh="true" data-show-toggle="true" data-show-columns="true" data-show-export="true" data-pagination="true" data-height="400" data-cache="false">
                       <thead>
                           <tr>
								<th data-field="code" data-align="left">ชื่อผู้ใช้งาน</th>
...
$(document).on("click", "a#search", function() {
	...
	if($('#codeCrt').val() == '') {
		...
		$('#codeCrt').removeAttr("style"); 
// 		$("#searchResultTable").bootstrapTable("refresh", { url: "../findAuthenByMsgCode.json?code="+ $("#codeCrt").val() });
// 		setTimeout(function () { $("#searchResultTable").bootstrapTable("resetView"); }, 200);
	...
	$('#codeCrt').removeAttr("style");
	$("#id").val(''); $("#code").val(''); $("#isPositive").val('');
	...
	runningMsgHide(); 
	$('#code').removeAttr("style"); $('#isPositive').removeAttr("style");
	$('#addOrEditForm')[0].reset();
}).on("click", "a#save", function() { 
	runningMsgHide(); 
	if($('#code').val() == '' && $('#isPositive').val() == '') {
