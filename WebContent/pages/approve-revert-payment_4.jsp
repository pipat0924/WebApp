	    ...
	    <div class="row">
	        <div class="col-md-12 tab-modefile">
               	<ol class="breadcrumb">
                    ...
                    <li class="active">ผลการอนุมัติขอคืนเงิน</li>
                </ol>
	        </div>
	    </div>
        <!--------------------------------------->
	    <ul id="navigatePanel" class="list-inline pull-right list-menu-set">
	        <li><a id="addRevertReceiptList" href="approve-revert-payment_1.jsp" class="btn btn-link"><span class="glyphicon glyphicon-arrow-left"></span> กลับไปอนุมัติขอคืนเงิน</a></li>
		...
		<div class="row">
			<div class="col-md-12 tab-modefile">
					...
					<table id="revertPaymentList4"
                       ...
                       <thead>
                           <tr>
                            <th data-field="revertApprNo" data-align="center">เลขที่อนุมัติขอคืนเงิน</th>
...
$(function(){
    var revertReqList3 = view.session("revertReqList3");
    var apprFlg = view.session("apprFlg");
    $('#revertPaymentList4').bootstrapTable('load', revertReqList3);
    try{
        if(apprFlg == 'Y'){
            window.open("../printRevertPaymentAP.pdf?reqRevNo="+revertReqList3[0].revertReqNo, "_blank");
        }
    }catch(err) {
        alert(err);
        console.log(err);
...
function docStatusFormatter(val, row, ind) {
    ...
    return '<span class="glyphicon glyphicon-ok-circle"></span>'+status;
}
