...
public class PaymentController {
	...
	public ExchangeRateDTO findCurrentExchangeRateJSON(
		...
		/*
		 * findExchRate(queryExchgRateDateString, exchangeRate); if(exchangeRate.getId()
		 * == null){ findExchRate(queryExchgRateString, exchangeRate); }
		 */
	...
	// W3P get symbol by currencyCode 20-01-2018 by got Sap
	@ResponseBody
	@RequestMapping(value = "getSymbolByCurrencyCodeSap.json", method = RequestMethod.GET)
	public ExchangeRateDTO getSymbolByCurrencyCodeSap(@RequestParam(value = "code", required = true) String currencyCode) {

		ExchangeRateDTO dto = new ExchangeRateDTO();

		String queryStr = " SELECT * FROM MASCHANGERATE WHERE RATECODE = '" + currencyCode + "' ";
	...
	public void findInvoiceByNoJSON(ModelMap modelMap, @RequestParam("invNo") String invNo) throws Exception {
		...
		if (_f01RetrieveInvoiceHeaderService.isCalledSuccesful("0", response)) {
			...
			for (int i = 0, m = invoiceHeaderListSize; i < m; i++) {
				...
				result.put("billGroup",
						billProfile != null
								? StringUtils.trimToEmpty(billProfile.getBillGroup()) + " "
										+ StringUtils.trimToEmpty(billProfile.getBillGroupFull())
								: null);
			...
			if (response.getInvoiceHeaderList() != null & response.getInvoiceHeaderList().size() > 0) {
				...
				for (InvoiceHeaderBO invoiceList : response.getInvoiceHeaderList()) {
					...
					Date duedate = invoiceList.getDueDate() != null
							? invoiceList.getDueDate().toGregorianCalendar().getTime()
							: null;
					...
					invoice.setTaxAmount(invoice.getAmountBeforeTax() == null ? BigDecimal.ZERO
							: invoice.getAmountBeforeTax().multiply(vatData).setScale(2, RoundingMode.HALF_UP));
					/*
					 * invoice.setAmountAfterTax(invoiceList.getTotal() == null ? BigDecimal.ZERO :
					 * BigDecimal.valueOf(invoiceList.getTotal()).divide(ONE_HUNDRED, 2,
					 * RoundingMode.HALF_UP));
					 */
					invoice.setAmountAfterTax(invoice.getAmountBeforeTax() == null ? BigDecimal.ZERO
							: invoice.getAmountBeforeTax().add(invoice.getTaxAmount()));
					/*
					...
					invoice.setIssueDate(invoiceList.getIssueDate() != null
							? invoiceList.getIssueDate().toGregorianCalendar().getTime()
							: null);
					invoice.setDueDate(duedate);
					invoice.setChargeCycleFromDate(invoiceList.getChargeFromDate() != null
							? invoiceList.getChargeFromDate().toGregorianCalendar().getTime()
							: null);
					invoice.setChargeCycleToDate(invoiceList.getChargeToDate() != null
							? invoiceList.getChargeToDate().toGregorianCalendar().getTime()
							: null);
					...
					invoice.setClosedate(invoiceList.getCloseDate() != null
							? invoiceList.getCloseDate().toGregorianCalendar().getTime()
							: null);
			...
			if (response.getWriteOffInvoiceList() != null & response.getWriteOffInvoiceList().size() > 0) {
				...
				for (WriteOffInvoiceBO invoiceList : response.getWriteOffInvoiceList()) {
					...
					Date duedate = invoiceList.getWriteOffDate() != null
							? invoiceList.getWriteOffDate().toGregorianCalendar().getTime()
							: null;
					chargeRequest.setBillRefNo(invoiceList.getBillRefNo());
					chargeRequest.setTransactionLog(_f03RetrieveInvoiceChargesService.buildTransactionLogCBO());
					RetrieveInvoiceChargeInfoResponse chargeResponse = _f03RetrieveInvoiceChargesService
					...
					invoice.setChargeCycleFromDate(invoiceList.getPpddDate() != null
							? invoiceList.getPpddDate().toGregorianCalendar().getTime()
							: null);
	...
	public InvoiceProductDTO findProductByInvoiceJSON(@RequestParam("no") String billRefNo,
		 ...
		 * buildTransactionLogCBO()); RetrieveInvoiceAccountCodeResponse response =
		 * _f02RetrieveInvoiceAccountCodeService.callInterface(request);
		 * 
		 * InvoiceProductDTO dto = new InvoiceProductDTO();
		 * if(_f02RetrieveInvoiceAccountCodeService.isCalledSuccesful("0", response)) {
		 * InvoiceProduct product; if (response.getInvoiceAccountCodeList() != null &
		 * response.getInvoiceAccountCodeList().size() > 0) { for (InvoiceAccountCodeBO
		 * output : response.getInvoiceAccountCodeList()) { product = new
		 * InvoiceProduct(String.valueOf(output.getBillRefNo()),
		 * output.getRevTypeCode(), output.getRevTypeName(), output.getProductCode(),
		 * output.getProductName(), output.getSubProductCode(),
		 * output.getSubProductName(), BigDecimal.valueOf(output.getAmount()).divide(new
	...
	public HistoryPaymentDTO findPaymentDetailsJSON(BillingInfo input) throws Exception {
		...
		try {
			...
			for (PaymentHistoryBO paymentHistory : response.getPaymentHistoryList()) {
				...
				/*
				 * <BillingAccountNo>800079218</BillingAccountNo> <BillRefNo>0</BillRefNo>
				 * <TrackingNo>14116</TrackingNo> <Amount>234</Amount> <vatAmount>
				 * <Status>ยกเลิก</Status> <ProcessDate>2011-05-03</ProcessDate>
				 * <PaymentType>Advance</PaymentType> billPeriod <LocationName>กองระบบบัญชี
				 * (ตัดหนี้สูญ)</LocationName> <PayDate>2011-05-03</PayDate>
				 */
				...
				detail.setTotalCharged(paymentHistory.getAmount() != null ? new BigDecimal(paymentHistory.getAmount())
						: BigDecimal.ZERO);
				detail.setVatAmount(
						paymentHistory.getVatAmount() != null ? new BigDecimal(paymentHistory.getVatAmount())
								: BigDecimal.ZERO);
				...
				detail.setProcessDate(paymentHistory.getProcessDate() != null
						? paymentHistory.getProcessDate().toGregorianCalendar().getTime()
						: null);
				...
				detail.setPaymentDate(paymentHistory.getPayDate() != null
						? paymentHistory.getPayDate().toGregorianCalendar().getTime()
						: null);
				if (paymentHistory.getBillRefNo().toString().equals("0"))
				...
				/*
				 * detail.setInvoiceNo(paymentHistory.getOrigBillRefNo(). toString());
				 * 
				 * //detail.setReceiptNo(receiptNo);
				 * //detail.setVatType(paymentHistory.getVatRate() != null ?
				 * paymentHistory.getVatRate().toString() : "");
				 * //detail.setTotalCharged(paymentHistory.getAmount() != null ? new
				 * BigDecimal(paymentHistory.getAmount() / 100.00) : BigDecimal.ZERO);
				 * 
				 * String receiptBranchName=""; if(paymentHistory.getLocationBO()!=null &&
				 * paymentHistory.getLocationBO().getLocationName()!=null)
				 * receiptBranchName=paymentHistory.getLocationBO(). getLocationName();
				 * detail.setReceiptBranchName(receiptBranchName );
				 * //detail.setUpdateUser(paymentHistory.getTransSource() != null ?
				 * paymentHistory.getTransSource().toString() : "");
				 * 
						...
						for (CreditLimitTrans creditLimitTrans : creditLimitTransList) {
							...
							if (amountIncVat != null && amountIncVat.length() > 0 && received != null
								...
								if (receivedD >= amountIncVatD && (creditLimitTrans.getArRef() != null
									...
									payType = "F";
									if ("Advance Payment".equals(creditLimitTrans.getArRef())) {
										payType = "A";
									}
							...
							if ((creditLimitTrans.getArRef() != null && !creditLimitTrans.getArRef().equals("-"))) {
								...
								if (inv != null) {
									if (inv.getCharge() != null)
										creditLimitTransEntity.setAmountExVat(
												String.valueOf(inv.getCharge().multiply(new BigDecimal(100))));
									if (inv.getVat() != null)
										creditLimitTransEntity.setVatAmount(
												String.valueOf(inv.getVat().multiply(new BigDecimal(100))));
									if (inv.getReceived() != null)
										creditLimitTransEntity.setAmountIncVat(
												String.valueOf(inv.getReceived().multiply(new BigDecimal(100))));
								}
							} else {
	...
	public CancelPaymentResultDTO cancelPaymentProductJSON(CancelPaymentDTO cancelPaymentDTO) throws Exception {
		...
		for (Receipt receipt : receipts) {
			...
			receipt.setCancelReason(rcp.getReasonDesc());
			receipt.setCancelCode(rcp.getReasonCode());
				...
				for (Invoice invoice : receipt.getInvoices()) {
					...
					for (Service service : invoice.getServices()) {
						if (service.getTransactions().size() > 0) {
							tList.addAll(service.getTransactions());
						}else {
							tList = transactionRepo.findByPayment(invoice.getPayment());
						}
	 ...
	 * public void getCouponDetail(HttpServletRequest request, HttpServletResponse
	 * response,
	 * 
	 * @RequestParam(value = "couponId") String couponId) throws Exception { //
	 * http://10.36.10.16:8080/insaleservices/coupons/coupon/couponId // String
	 * jsonStringResult = // AppUtil.callURL(
	 * "http://10.44.1.4/InsaleManagement/Services_MS?table=import_coupon&type=list&id="
	 * +couponId); String jsonStringResult = AppUtil
	 * .callURL("http://10.36.10.16:8080/insaleservices/coupons/coupon/" +
	 * couponId.trim()); System.out.println("jsonStringResult: " +
	 * jsonStringResult); AppUtil.drawJson(request, response, jsonStringResult); }
	 */
	@ResponseBody
	@RequestMapping(value = "/getCouponDetail.json", method = RequestMethod.GET)
	// public void getCouponDetail(HttpServletRequest request, HttpServletResponse
	...
	public th.co.softpos.ws.mg.s001.Response getCouponDetail(HttpServletRequest request, HttpServletResponse response,
		...
		// _response.
		// String jsonStringResult =
		// AppUtil.callURL("http://10.36.10.16:8080/insaleservices/coupons/coupon/"+couponId.trim());
	...
	public CreatePaymentResultDTO createPaymentPenaltyExtendJSON(ModelMap modelMap, SettlePaymentDTO paymentDTO)
		...
		// jsonObject.getString(TOPUP_FIELD_SERVICE_KEY));
		/*
		 * String authTokens = (String) modelMap.get(TOPUP_FIELD_AUTH_TOKEN); for
		 * (SettlePaymentDTO.Customer customer : paymentDTO.getCustomers()) { for
		 * (SettlePaymentDTO.Service service : customer.getServices()) { String
		 * refTransId=AppUtil.generateTransactionID(9);
		 * service.setRefTransId(refTransId); JSONObject verifyResult =
		 * topupService.verifyTopup(authTokens, service.getCode(), service.getNo(),
		 * service.getSubscriber(), EMPTY, service.getRefTransId(),
		 * service.getAmount().toPlainString()); String status_code =
		 * (String)verifyResult.get("status_code"); //System.out.println(status_code);
		 * //JSONObject confirmResult = topupService.confirmTopup(authTokens,
		 * service.getCode(), service.getNo(), service.getSubscriber(), EMPTY,
		 * verifyResult.getString(TOPUP_FIELD_REF_TRANSID),
		 * service.getAmount().toPlainString()); //JSONObject confirmResult =
		 * topupService.confirmTopup(authTokens, service.getCode(), service.getNo(),
		 * service.getSubscriber(), EMPTY, "1234", service.getAmount().toPlainString());
		 * } }
		 */
	...
	public PaymentHistoryDTO findPaymentHistoryReceipt(@Param("no") String no, @Param("invNo") String invNo,
			@Param("payType") String payType, Pageable p) throws SOAPException, IOException,
			UnsupportedOperationException, ParserConfigurationException, SAXException {
		PaymentHistoryDTO dto = new PaymentHistoryDTO();
	...
	public String checkPromotionServerStatus(@RequestParam("accounts") String accounts, @RequestParam("ip") String ip) {
		...
		if (accounts != null && accounts.length() > 0) {
			List<PromotionMappingEntity> promotionMappingList = promotionMappingRepository
					.findInActivePromotionMappingNotInAccount(accountList);
	...
	public String checkServerStatusForCancel(@RequestParam("ip") String ip, @RequestParam("no") String no) {
		...
		if (status.equals("OK")) {
			...
			if (receipts != null && receipts.size() > 0) {
				...
				if (receiptId != null) {
					PromotionReceiptMappingEntity promotionReceiptMappingEntity = promotionBillingMappingRepository
							.findPromotionReceiptMappingByReceiptid(receiptId);
